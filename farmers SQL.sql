...
SELECT REVIEW_TITLE
FROM "REVIEW"
WHERE REVIEW_MENU LIKE '%빵%'
OR REVIEW_ADDRESS LIKE '%서울%'
OR ();
...
JOIN "TOGETHER" T ON (C.CHATTING_NO = T.BOARD_NO)
WHERE PARTICIPANT = #{MEMBERNO}
OR OPEN_MEMBER = #{MEMBERNO};

-- 내가 개설했거나 또는 내가 참여자로 있는 채팅방의 보드이미지를 가져옴
...
SELECT CHATTING_NO
			...
			,(SELECT COUNT(*) FROM MESSAGE M WHERE M.CHATTING_NO = R.CHATTING_NO AND READ_FL = 'N' AND SENDER_NO != 41) NOT_READ_COUNT --안읽은 메세지--
			,(SELECT MAX(MESSAGE_NO) SEND_TIME FROM MESSAGE M WHERE R.CHATTING_NO  = M.CHATTING_NO) MAX_MESSAGE_NO --마지막 메세지, sender 없는거 해결하새요--

		FROM CHATTING_ROOM R
		WHERE OPEN_MEMBER = 41
		OR PARTICIPANT = 41
...
DROP TABLE "TOGETHER_REPORT";
DROP SEQUENCE SEQ_TOGETHER_REPORT;
COMMIT;

CREATE TABLE "TOGETHER_REPORT" (
	"T_REPORT_NO"	NUMBER		NOT NULL,
	"T_REPORT_CONTENT"	VARCHAR2(300)		NOT NULL,
	"T_REPORT_DEL_FL"	VARCHAR(255)	DEFAULT 'N'	NOT NULL,
	"BOARD_NO"	NUMBER		NOT NULL
);

COMMENT ON COLUMN "TOGETHER_REPORT"."T_REPORT_NO" IS '신고 번호(SEQ_REPORT_NO)';

COMMENT ON COLUMN "TOGETHER_REPORT"."T_REPORT_CONTENT" IS '신고 내용';

COMMENT ON COLUMN "TOGETHER_REPORT"."T_REPORT_DEL_FL" IS '삭제 여부';

COMMENT ON COLUMN "TOGETHER_REPORT"."BOARD_NO" IS '게시글번호(SEQ_BOARD_NO)';

ALTER TABLE "TOGETHER_REPORT" ADD CONSTRAINT "PK_UNTITLED3" PRIMARY KEY (
	"T_REPORT_NO"
);

ALTER TABLE "TOGETHER_REPORT" ADD CONSTRAINT "FK_TOGETHER_TO_TOGETHER_REPORT_1" FOREIGN KEY (
	"BOARD_NO"
)
REFERENCES "TOGETHER" (
	"BOARD_NO"
...
CREATE SEQUENCE SEQ_TOGETHER_REPORT NOCACHE;

SELECT * FROM TOGETHER_REPORT;

-- 신고 누적 조회 수 조회
SELECT COUNT(REPORT_NO) FROM "REVIEW_REPORT" GROUP BY REVIEW_NO;

		SELECT REVIEW_NO, REVIEW_TITLE, MEMBER_NICKNAME,
		TO_CHAR(WRITE_DATE , 'YYYY"년" MM"월" DD"일" HH24"시" MI"분" SS"초"') WRITE_DATE,
		(SELECT COUNT(REPORT_NO) FROM "REVIEW_REPORT" GROUP BY REVIEW_NO) ACCRUE
		FROM REVIEW R
		JOIN "MEMBER" USING (MEMBER_NO)
		ORDER BY REVIEW_NO;
